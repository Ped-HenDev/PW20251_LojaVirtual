...
def inserir_categorias(categoria: Categoria) -> Categoria:
...
def atualizar_categorias(categoria: Categoria) -> bool:
...
def excluir_categorias(id: int) -> bool:
...
def obter_categorias_por_id(id: int) -> Categoria:
...
def obter_categorias_por_pagina(limite: int, offset: int) -> list[Categoria]:
    """Obtém categorias por página (com paginação)."""
    conexao = obter_conexao()
    cursor = conexao.cursor()    
    cursor.execute(GET_CATEGORIAS_BY_PAGE, (limite, offset))
    resultados = cursor.fetchall()
    categorias = [Categoria(id=resultado[0], nome=resultado[1]) for resultado in resultados]
    conexao.close()    
    return categorias
